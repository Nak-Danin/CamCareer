...
@endphp
<main class="flex flex-col gap-4 md:gap-2 border-2 border-gray-300 bg-white rounded p-4 hover:border-blue-600 cursor-pointer group">
    ...
    <div class="flex justify-between items-center gap-4">
        <div class="flex gap-3">
            ...
            <span class="hidden md:flex text-description text-sm capitalize text-blue-800 px-4 bg-indigo-100 justify-center items-center font-medium rounded">{{ $job->career_type }}</span>
        </div>
        @if (!$isAlreadyApplied)
    ...
    <div class="flex items-center justify-between">
        <div class="flex gap-4 items-center text-gray-700">
            <span class="hidden md:block"><i class="fi fi-rs-building"></i> {{ $job->company->company_name }}</span>
            ...
            <span class="hidden md:block"><i class="fi fi-rs-marker"></i> {{ $job->location }}</span>
            <span><i class="fi fi-rr-payroll-check pt-2"></i> {{ $salary_range }}</span>
        </div>
