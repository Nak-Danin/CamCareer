<x-master-layout>
    <main
        ...
        class="flex flex-col gap-6 md:gap-10 justify-center items-center p-5 md:p-20">
        ...
        <section class="flex gap-4 items-center w-[95%] md:w-[60%] border border-gray-300 px-5 py-3 bg-white text-gray-800">
            ...
            <input x-model="search" class="hidden md:block w-full focus:border-0 focus:ring-0 focus:outline-none md:text-lg" type="text" placeholder="Search categories, industries, or keywords...">
            <input x-model="search" class="md:hidden w-full focus:border-0 focus:ring-0 focus:outline-none md:text-lg" type="text" placeholder="Search categories, industries...">
        </section>
        <section class="grid grid-cols-2 md:grid-cols-4 justify-evenly gap-4 w-full">
            @foreach ($categories as $category)
            @php
            $jobCount = $category->careers->where('status', 'available')->count();
            @endphp
            <div
        ...
        <!-- No Results -->
        <section
            x-show="search && !categories.some(category => category.name.includes(search.toLowerCase()))"
