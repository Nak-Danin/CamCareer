    ...
    <div class="flex h-screen items-stretch">
        ...
        <div class="right-panel flex-1 flex items-center justify-center px-6 py-12 bg-white">
            <div class="w-full max-w-xl h-full bg-white">
                <h1 class="text-2xl font-medium text-gray-900 mb-1">Create Account</h1>
                <p class="text-sm text-gray-500 mb-6">Get started with your career journey today.</p>

                {{-- Role toggle --}}
                <div class="flex border border-gray-200 rounded-lg overflow-hidden mb-5">
                    <button type="button" id="btn-seeker" onclick="setRole('seeker')"
                        class="flex-1 py-2 text-sm font-medium transition-colors bg-[#1a3a8f] text-white">
                        Job Seeker
                    </button>
                    <button type="button" id="btn-employer" onclick="setRole('employer')"
                        class="flex-1 py-2 text-sm font-medium transition-colors bg-transparent text-gray-500">
                        Employer
                    </button>
                </div>
                ...
                <form method="POST" action="{{ route('auth.store') }}">
                    ...
                    {{-- Seeker: first + last name --}}
                    <div id="seeker-fields" class="flex gap-3 mb-3">
                        <div class="flex-1">
                            <label for="first_name" class="block text-xs text-gray-500 mb-1">First Name</label>
                            <input type="text" id="first_name" name="first_name"
                                value="{{ old('first_name') }}"
                                placeholder="e.g. Sok"
                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1a3a8f]/30 focus:border-[#1a3a8f] transition @error('first_name') border-red-400 @enderror">
                            @error('first_name')
                        ...
                        <div class="flex-1">
                            ...
                            <input type="text" id="last_name" name="last_name"
                                ...
                                class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1a3a8f]/30 focus:border-[#1a3a8f] transition @error('last_name') border-red-400 @enderror">
                            @error('last_name')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div id="employer-fields" class="hidden mb-3">
                        <label for="company_name" class="block text-xs text-gray-500 mb-1">Company Name</label>
                        <input type="text" id="company_name" name="company_name"
                            value="{{ old('company_name') }}"
                            placeholder="e.g. Phnom Penh Tech Co."
                            class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1a3a8f]/30 focus:border-[#1a3a8f] transition @error('company_name') border-red-400 @enderror">
                        @error('company_name')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="relative">
                            <input type="password" id="password_confirmation" name="password_confirmation"
                                ...
                                class="w-full px-3 py-2 pr-9 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1a3a8f]/30 focus:border-[#1a3a8f] transition @error('password_confirmation') border-red-400 @enderror">
                            <button type="button" onclick="togglePwd('password_confirmation', 'eye2')"
                                class="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                                <svg id="eye2" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                </svg>
                            </button>
                ...
                <p class="text-center text-sm text-gray-500 mt-4">
                    ...
                    <a href="{{ route('login') }}" class="text-[#1a3a8f] font-medium hover:underline">Login</a>
                </p>
                <p class="text-center text-xs text-gray-400 mt-2">
                    By signing up, you agree to CamCareer
